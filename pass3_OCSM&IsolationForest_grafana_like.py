...
if __name__ == '__main__':
    ...
    try:
        ...
        model = joblib.load(ocsvm_model_output_path)

        X_all_features = df_processed_original[features_columns]
        X_all_scaled = scaler.transform(X_all_features)
        
        df_processed_original['ocsvm_anomaly_score'] = model.decision_function(X_all_scaled)
        df_processed_original['ocsvm_anomaly_prediction'] = model.predict(X_all_scaled)
    ...
    try:
        ...
        # Predict anomaly scores for the *entire* processed dataset using IF
        X_all_features = df_processed_original[features_columns]
        X_all_scaled = scaler.transform(X_all_features)
        
        df_processed_original['if_anomaly_score'] = if_model.decision_function(X_all_scaled)
        df_processed_original['if_anomaly_prediction'] = if_model.predict(X_all_scaled)
