...
def detect_anomaly_realtime(
    ...
    try:
        ...
        # print(f"Loaded scaler from {scaler_path} and model from {model_path}") # Suppress for bulk processing
...
if __name__ == '__main__':
    ...
    try:
        ...
        ocsvm_model = OneClassSVM(kernel='rbf', nu=0.01, gamma='scale') # Using nu=0.01 as in previous successful run
    ...
    pred, score = detect_anomaly_realtime(simulated_anomalous_data) # This will print "Loaded scaler..." again
    ...
    # --- Generating Grafana-like Output (Static Plot) ---
    print("\n--- Generating Grafana-like Anomaly Visualization ---")

    # Load scaler and model once for bulk prediction
    try:
        scaler = joblib.load('standard_scaler.joblib')
        model = joblib.load('one_class_svm_model.joblib')

        # Predict anomaly scores for the *entire* processed dataset
        X_all_features = df_processed_original[features_columns]
        X_all_scaled = scaler.transform(X_all_features)
        
        df_processed_original['anomaly_score'] = model.decision_function(X_all_scaled)
        df_processed_original['anomaly_prediction'] = model.predict(X_all_scaled)
        
        # Identify anomalies based on prediction
        anomalies_df = df_processed_original[df_processed_original['anomaly_prediction'] == -1]
        ...
        # Highlight detected anomalies in the CPU plot
        plt.scatter(anomalies_df['Timestamp'], anomalies_df['CPUUtilizationPercent'], 
                    color='red', s=50, label='Detected Anomaly', zorder=5, marker='X')
        
        plt.title('CPU Utilization with Detected Anomalies (Grafana-like View)', fontsize=16)
        ...
        sns.lineplot(x='Timestamp', y='anomaly_score', data=df_processed_original, hue='ContainerID', linewidth=0.8, alpha=0.7, palette='tab10', legend=False)
        
        # Plot the anomaly threshold (0 for OCSVM decision function)
        plt.axhline(y=0, color='grey', linestyle='--', label='Anomaly Threshold (0)')
        
        # Highlight detected anomalies in the Anomaly Score plot
        plt.scatter(anomalies_df['Timestamp'], anomalies_df['anomaly_score'], 
                    color='red', s=50, label='Detected Anomaly', zorder=5, marker='X')
        ...
        plt.suptitle("Simulated Anomaly Dashboard View", y=1.02, fontsize=20) # Overall title
        plt.show()
